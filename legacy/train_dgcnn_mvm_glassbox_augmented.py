...
# -*- coding: utf-8 -*-
"""
训练DGCNN + MvM模型在glassbox类别上（带12旋转增强）

详细说明：
- 模型/方法: DGCNN backbone + MvM预测头（K=4峰，固定）
...
使用方法：
    python train_dgcnn_mvm_glassbox_augmented.py

输出位置：
    results/dgcnn_glassbox_YYYYMMDD_HHMMSS/
...
from models.dgcnn_mvM import DGCNNMvM
...
# 实验命名
EXP_NAME = f"dgcnn_glassbox_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
...
# 超参数
NUM_POINTS = 2048  # 减少点数以加快DGCNN训练
BATCH = 16  # 点数减少后可以增加batch size
EPOCHS = 50
LR = 5e-4
SEED = 42

# DGCNN 特定参数
DGCNN_K = 20  # k nearest neighbors
DGCNN_EMB_DIMS = 1024  # embedding dimension
...
with open(config_txt, "w") as f:
    ...
    f.write(f"Model: DGCNN + MvM\n")
    f.write(f"DGCNN_K: {DGCNN_K}\n")
    f.write(f"DGCNN_EMB_DIMS: {DGCNN_EMB_DIMS}\n\n")
...
# ============ 可视化函数 ============
def plot_loss_curve(xs, train_vals, val_vals, out_path, title="DGCNN GlassBox Training Loss"):
    """绘制训练/验证loss曲线"""
...
def main():
    print(f"\n{'='*60}")
    print(f"  DGCNN GlassBox Training - 4-Peak von Mises Mixture")
    print(f"{'='*60}\n")
    ...
    model = DGCNNMvM(
        ...
        temp=0.7,
        k=DGCNN_K,
        emb_dims=DGCNN_EMB_DIMS
    ).to(device)

    # 打印模型参数量
    total_params = sum(p.numel() for p in model.parameters())
    trainable_params = sum(p.numel() for p in model.parameters() if p.requires_grad)
    print(f"[Model] DGCNN+MvM - Total params: {total_params:,}, Trainable: {trainable_params:,}")
