...
'''
    ...
    def _load_annotations(self):
        ...
        if self.output_file.exists():
            # 记录文件修改时间
            self._file_mtime = self.output_file.stat().st_mtime
    ...
    def _check_and_sync_external_changes(self):
        """检测并同步外部文件修改"""
        ...
        if current_mtime > self._file_mtime:
            # 文件被外部修改了
            print(f"[Sync] 检测到外部修改，正在同步...")

            # 读取外部文件
            with open(self.output_file, 'r') as f:
                external_annotations = json.load(f)

            # 智能合并：保留当前会话的修改，同时加入外部新增的标注
            merged = external_annotations.copy()
            ...
            old_count = len(self.annotations)
            ...
            print(f"[Sync] 同步完成: {old_count} -> {new_count} 条标注")
            return True
        ...
        def handle_all_interactions(*args):
            ...
            if ctx.triggered:
                ...
                if button_id == 'save-interval':
                    # 先检测外部修改
                    synced = self._check_and_sync_external_changes()
                    if synced:
                        save_msg = f"🔄 已同步外部修改 (共{len(self.annotations)}条)"
