...
# -*- coding: utf-8 -*-
"""
训练PointNet++ + MvM模型在glassbox类别上（修复weight不平衡问题）

改进点：
1. ✅ 温度参数temp从0.7改为2.0（平滑权重分布）
2. ✅ 添加weight loss优化（KL散度 + weight匹配）
3. ✅ 12旋转增强 + 点云抖动
4. ✅ 预设角度初始化打破对称性

详细说明：
- 模型/方法: PointNet++ backbone + MvM预测头（K=4峰，temp=2.0）
- 数据集: ModelNet40 glassbox类别 (271样本)
- 数据增强: 12旋转增强（30°间隔）+ 点云抖动
- 训练策略: KL散度 + Weight KL散度 + Hungarian匹配
- 输出: 模型checkpoints、训练日志、可视化

使用方法：
    python train_pointnetpp_mvm_glassbox_fixed_weight.py

输出位置：
    results/glassbox_fixed_weight_YYYYMMDD_HHMMSS/
...
作者: Claude
创建日期: 2025-11-13
关联文档: weight_imbalance_analysis.md
实验ID: exp_20251113_weight_fix
"""
import time
import random
import math
...
# 实验命名
EXP_NAME = f"glassbox_fixed_weight_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
...
with open(config_txt, "w") as f:
    ...
    f.write(f"=== 改进点 ===\n")
    f.write(f"1. Temperature: 2.0 (之前0.7导致weight集中)\n")
    f.write(f"2. Weight Loss: 添加weight KL散度优化\n")
    f.write(f"3. Loss权重: KL + 0.1*WeightKL\n\n")
...
def match_loss(mu_pred, kappa_pred, w_pred, vm_gt, K_gt, return_matching=False):
    """
    基于匈牙利算法的匹配loss
    ...
    Args:
        ...
        K_gt: (B,) 每个样本的实际峰数量
        return_matching: 是否返回匹配信息（用于weight loss）

    Returns:
        loss_vec: (B,) 每个样本的KL loss
        matching_info: (可选) 如果return_matching=True，返回匹配的行列索引
    """
    B = mu_pred.size(0)
    loss_vec = torch.zeros(B, device=device)
    matching_info = [] if return_matching else None
    ...
    for b in range(B):
        ...
        if return_matching:
            matching_info.append((row, col))

    if return_matching:
        return loss_vec, matching_info
    return loss_vec


def weight_loss(w_pred, vm_gt, K_gt, matching_info):
    """
    Weight匹配loss：优化预测的weight与GT weight的一致性

    Args:
        w_pred: (B, max_K) 预测的weight
        vm_gt: (B, max_K, 3) GT的[mu, kappa, weight]
        K_gt: (B,) 每个样本的实际峰数量
        matching_info: list of (row, col) tuples from Hungarian matching

    Returns:
        loss_vec: (B,) 每个样本的weight loss
    """
    B = w_pred.size(0)
    loss_vec = torch.zeros(B, device=device)
...
def main():
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        for xyz, vm_gt, K, _ in train_loader:
            ...
            # 计算loss（KL loss + Weight loss）
            kl_loss_vec, matching_info = match_loss(mu_pred, kappa_pred, w_pred, vm_gt, K, return_matching=True)
            w_loss_vec = weight_loss(w_pred, vm_gt, K, matching_info)

            # 总loss（KL loss主导，weight loss为辅助）
            ...
            train_loss += loss.item() * xyz.size(0)
