...
# -*- coding: utf-8 -*-
"""
训练PointNet++ + MvM模型在glassbox类别上（无数据增强 - 消融实验）

详细说明：
- 模型/方法: PointNet++ backbone + MvM预测头（K=4峰，固定）
- 数据集: ModelNet40 glassbox类别（仅原始217个训练样本，无增强）
- 数据增强: **无** (对比实验：验证旋转增强的作用)
- 训练策略: KL散度loss + Hungarian匹配，Adam优化器，学习率衰减
- 输出: 模型checkpoints、训练日志、可视化（极坐标图）

实验目的：
消融实验（Ablation Study）- 对比有无数据增强的效果
- 实验组: train_pointnetpp_mvm_glassbox_augmented.py (12旋转，2604样本)
- 对照组: 本脚本 (无旋转，217样本)

预期假设：
无增强版本由于数据量少（217样本），可能出现：
1. 过拟合（train loss低但val loss高）
2. 泛化能力差
3. 收敛速度慢或不收敛

核心特性：
1. 预设角度初始化 [0°, 90°, 180°, 270°] 打破对称性（与增强版相同）
2. 仅使用原始217个训练样本
3. 每10个epoch保存可视化
...
输出位置：
    results/glassbox_no_augment_YYYYMMDD_HHMMSS/
...
创建日期: 2025-11-09
最后修改: 2025-11-09
关联文档: (待创建) experiment_20251109_no_augment_ablation_results.md
实验ID: exp_20251109_no_augment
对比实验: train_pointnetpp_mvm_glassbox_augmented.py
"""
import time
import random
import math
...
# 实验命名
EXP_NAME = f"glassbox_no_augment_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
...
# 数据增强配置（消融实验：完全禁用）
ROTATION_ANGLES = [0]  # 仅保留0度，即无旋转增强
APPLY_JITTER = False   # 不添加点云抖动
JITTER_STD = 0.0       # 抖动标准差设为0
