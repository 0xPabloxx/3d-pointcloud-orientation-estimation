...
# -*- coding: utf-8 -*-
"""
DGCNN → Mixture of von Mises (2D yaw) 预测网络

与 pointnet_pp_mvM.py 结构对应，使用 DGCNN 作为 backbone 替代 PointNet++
用于性能对比实验。
"""
...
class DGCNNMvM(nn.Module):
    """
    DGCNN backbone + 混合 von Mises 参数头（2D 平面角度）
    ...
    def __init__(
        ...
        p_drop: float = 0.4,
        temp: float = 2.0,
        k: int = 20,  # DGCNN k nearest neighbors
        emb_dims: int = 1024,  # DGCNN embedding dimension
    ):
        super().__init__()
        self.max_K = max_K
        ...
        # ---------- DGCNN Backbone ----------
        self.backbone = DGCNN(k=k, emb_dims=emb_dims, dropout=p_drop)

        # DGCNN outputs emb_dims*2 (max + avg pooling)
        backbone_out_dim = emb_dims * 2
    ...
    def _global_feat(self, xyz: torch.Tensor) -> torch.Tensor:
        """Extract global features using DGCNN backbone."""
        # DGCNN expects (B, 3, N)
        feat = self.backbone(xyz)  # (B, emb_dims*2)
