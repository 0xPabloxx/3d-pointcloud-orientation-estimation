...
# 8 个水平方向(角度 0,45,…,315°)，顺时针从 “原始正向” [0,0,-1] 开始
DIRS_8 = torch.tensor([
    [ 0.0000, 0.0, -1.0000],   # 0°   (南)
    [ 0.7071, 0.0, -0.7071],   # 45°  (西南)
    [ 1.0000, 0.0,  0.0000],   # 90°  (西)
    [ 0.7071, 0.0,  0.7071],   # 135° (西北)
    [ 0.0000, 0.0,  1.0000],   # 180° (北)
    [-0.7071, 0.0,  0.7071],   # 225° (东北)
    [-1.0000, 0.0,  0.0000],   # 270° (东)
    [-0.7071, 0.0, -0.7071],   # 315° (东南)
])                               # shape (8,3)


class PointNetPP8Dir(nn.Module):
    """
    PointNet++ backbone + 8-way direction head.
    输出为 8 维 logits，经 softmax 后为各基向量的概率。
    """
    def __init__(self):
        super().__init__()
        ...
        self.fc3 = nn.Linear(256, 8)        # ---- 改为 8 维 ----
