...
class PointCloudScreenshotViewer:
    ...
    def _build_layout(self):
        ...
        self.app.layout = dbc.Container([
            ...
            # 存储当前相机位置（用户手动旋转后的）
            dcc.Store(id='current-camera-store', data=None),
    ...
    def _setup_callbacks(self):
        ...
        def update_display(file_path, color_scheme, solid_color, bg_color, point_size,
            ...
            # 使用uirevision保持用户的相机旋转状态
            # 只有当相机预设改变或文件改变时才重置相机
            camera_triggers = ['camera-selector', 'btn-align-front', 'btn-align-side', 'btn-align-top',
                              'file-selector', 'btn-prev', 'btn-next', 'zoom-value']
            if trigger in camera_triggers:
                # 重置相机到预设位置
                ui_revision = f"{current_file}_{new_camera}_{zoom}"
            else:
                # 保持用户手动旋转的相机位置
                ui_revision = f"{current_file}_keep"
        ...
        # 捕获用户手动旋转后的相机位置
        @self.app.callback(
            Output('current-camera-store', 'data'),
            [Input('pointcloud-3d', 'relayoutData')],
            [State('current-camera-store', 'data')],
            prevent_initial_call=True
        )
        def store_camera_position(relayout_data, current_store):
        ...
        def save_screenshot(save_clicks, batch3_clicks, batch_iso_clicks,
                           ...
                           camera, render_opts, align_opts, zoom_value, custom_camera):
            ctx = callback_context
            ...
            if trigger == 'btn-save':
                ...
                # 如果有用户手动旋转的相机位置，使用它
                if custom_camera:
                    fig.update_layout(scene_camera=custom_camera)
                    out_path = self.output_dir / f"{file_prefix}_custom.png"
                else:
